    ...
    try {
        ...
        for (int plate_index = 0; plate_index < plate_count; plate_index++)
        {
            ...
            for (int object_index = 0; object_index < object_count; object_index++)
            {
                ...
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_SCALE_X)) {
                    assemble_object.scale_x = object_json[JSON_ASSEMPLE_OBJECT_SCALE_X].get<std::vector<float>>();
                    if ((assemble_object.scale_x.size() > 0) && (assemble_object.scale_x.size() != assemble_object.count) && (assemble_object.scale_x.size() != 1))
                    {
                        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(": object %1%'s pos_x count %2% not equal to clone count %3%, also not equal to 1") % assemble_object.path % assemble_object.scale_x.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_SCALE_Y)) {
                    assemble_object.scale_y = object_json[JSON_ASSEMPLE_OBJECT_SCALE_Y].get<std::vector<float>>();
                    if ((assemble_object.scale_y.size() > 0) && (assemble_object.scale_y.size() != assemble_object.count) && (assemble_object.scale_y.size() != 1))
                    {
                        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(": object %1%'s pos_y count %2% not equal to clone count %3%, also not equal to 1") % assemble_object.path % assemble_object.scale_y.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_SCALE_Z)) {
                    assemble_object.scale_z = object_json[JSON_ASSEMPLE_OBJECT_SCALE_Z].get<std::vector<float>>();
                    if ((assemble_object.scale_z.size() > 0) && (assemble_object.scale_z.size() != assemble_object.count) && (assemble_object.scale_z.size() != 1))
                    {
                        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(": object %1%'s pos_z count %2% not equal to clone count %3%, also not equal to 1") % assemble_object.path % assemble_object.scale_z.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
...
static int construct_assemble_list(std::vector<assemble_plate_info_t> &assemble_plate_info_list, Model &model, PlateDataPtrs &plate_list)
{
    ...
    for (int index = 0; index < plate_count; index++)
    {
        ...
        for (size_t obj_index = 0; obj_index < object_count; obj_index++)
        {
            ...
            object->scale(assemble_object.scale_x[0], assemble_object.scale_y[0], assemble_object.scale_z[0]);
            ...
            for (size_t copy_index = 1; copy_index < assemble_object.count; copy_index++)
            {
                ...
                object->scale(assemble_object.scale_x[array_index], assemble_object.scale_y[array_index], assemble_object.scale_z[array_index]);
